...
**Joints:** pose names live on the **Skeleton3D**. `MeshPoseRig` undoes the 0.01 armature so the authored 100× IBM skins `Body` at 1.8 m (do not replace binds with inverse(rest) — that explodes vertices). Bind-pose chest is Bip01 +X; the authored −90° Y leaves that on MeshRoot +Z, so the rig adds π (+90°) to match MeshRoot −Z / camera-relative WASD. Idle is the bind A-pose. Jab / heavy fold the forearm and cap `aim_along_y` (a 90° upper-arm swing stretches the IBM sleeves — that is a pose/skin error, not a reach buff). Walk / sprint use rest-relative local-Z extras on thighs and shins (thigh local X is MeshRoot forward, so Euler X abducts and reads as a planted slide). Aim-style remaps and hips-basis remaps explode the IBM skin.
